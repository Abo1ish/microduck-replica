...
# 1 舵机通信协议
舵机使用FT-SCS自定义协议，出厂时的串口默认配置，SMS舵机波特率默认为115200使用RS485总线通信；波特率可配置范围38400~1Mbps，默认通信地址（站号）为1。
...
|7|0x07|应答返回延时|1|0|读写|0/50 ~ 253|2us|最大可设置返回延时 254*2=508us，0表示最少返回延时，设置<50默认仍然为50（100us)|
...
地址DEC|地址HEX|功能名称|字节数|初始值|权限|取值范围|单位|说明|
|-|-|-|-|-|-|-|-|-|
|40|0x28|扭矩开关|1|0|读写|0 ~ 2|无|写0：关闭扭力输出；写1：打开扭力输出；写128：任意当前位置较正为2048|
...
- BIT5（32）：电压采样，(0)1.5K低压采样，(1)1K高压采样
