...
# 1 舵机通信协议
舵机使用FT-SCS自定义协议，出厂时的串口默认配置，STS舵机默认波特率为1M使用TTL单总线通信，数据位 8，无校验，停止位 1；波特率可配置范围38400~1Mbps，默认通信地址（站号）为1。
...
|21|0x15|位置环P 比例系数|1|--|读写|0 ~ 254|无|控制电机的比例系数(1/4)|
|22|0x16|位置环D 微分系数|1|--|读写|0 ~ 254|无|控制电机的微分系数(1/8)|
|23|0x17|位置环I 积分系数|1|0|读写|0 ~ 254|无|控制电机的积分系数|
|24|0x18|最小启动扭矩|1|--|读写|0 ~ 254|0.1%|设置舵机的最小输出启动扭矩|
...
|33|0x21|运行模式|1|0|读写|0 ~ 3|无|0：位置伺服模式；1：电机恒速模式；2：PWM开环调速度模式；3：步进模式|
...
地址DEC|地址HEX|功能名称|字节数|初始值|权限|取值范围|单位|说明|
|-|-|-|-|-|-|-|-|-|
|40|0x28|扭矩开关|1|0|读写|0 ~ 2|无|写0：关闭扭力输出；写1：打开扭力输出；写2：阻尼输出（转矩限制（48）可控制阻尼力大小），写128：任意当前位置较正为2048|
...
|81|0x51|DTs(ms)|1|--|只读|--|--|--|
|82|0x52|速度单位系数|1|--|只读|--|--|--|
|83|0x53|最小速度限制|1|--|只读|--|--|单位0.732rpm|
...
- BIT5（32）：驱动桥配置，(0)独立H桥，(1)集成H桥/无电流反馈
- BIT6（64）：PWM频率，(0)24kHz，(1)16kHz
- BIT7（128）：位置反馈方向相位，(0)正向，(1)反向
- <div style="padding: 8px 16px;background-color: #fff9e9;border-radius: 4px;border-left: 5px solid #fbc302;margin: 20px 0;">如果同时设置多个位，舵机的相位值为各个位值之和。例如：原相位值为0，舵机运行反向，则相位值为128+1=129；
</div> 
...
- <div style="padding: 8px 16px;background-color: #fff9e9;border-radius: 4px;border-left: 5px solid #fbc302;margin: 20px 0;">如果同时设置多个位，舵机的LED报警条件值为各位值之和。例如：电压报警与过热报警同时开启，则报警条件值为4+1=5；
</div>
