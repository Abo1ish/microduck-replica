...
本目录提供源码、Keil 工程，以及网页读取所匹配的 `Build/imu_to_dxl.bin`、`.hex` 和 `.map`。
预编译 BIN 的 SHA-256 为 `69bdaea7224900addd6eb5bd28001e26e07073c129383d9c1ece380d2bfce19a`。
仅使用姿态网页时无需重新编译；改动固件后需要重新核对网页读取的 RAM 布局与固件校验值。
本仓库不附带开发者的 `.axf` 调试产物，源码断点调试应在自己的环境重新构建。
...
不代表已经满足仓库最新 `hardware/imu_to_dxl/总线协议.md` 的15字节飞特协议验收，也不保证与 `tools/imu200` 直接互通。
...
### J3 接线必须以这版 PDF 和实板网络为准

| J3 脚号 | 最新原理图信号 | 接法 |
|---|---|---|
| 1 | GND | ST-LINK/USB-TTL 共地 |
| 2 | SWDCLK | ST-LINK SWCLK |
| 3 | SWDIO | ST-LINK SWDIO |
| 4 | UART_RX，U1封装脚17 | USB-TTL TX，3.3V 电平 |
| 5 | UART_TX，U1封装脚16 | USB-TTL RX，3.3V 电平 |
| 6 | 3.3V | 调试器电压参考；不要把 5V 接到这里 |

旧接线表的 J3.4/5 与这版原理图相反，旧 `+3V3` 网络描述也有残留错误。
首次上电先按焊盘/EDA 网络核实，勿照旧文字把 J3.4 当电源。
