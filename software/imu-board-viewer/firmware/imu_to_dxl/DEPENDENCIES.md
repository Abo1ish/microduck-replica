...
| Arm Compiler for Embedded | 验证版本6.24 | 需有效工具授权；使用 Arm Compiler 6、旧语法 armasm 和 MicroLIB，仅隐藏旧汇编器弃用通知 A1950W |
...
构建会替换 `Build` 中的固件；仅使用姿态网页时应保留随工程提供的 `.bin/.hex/.map`，无需重新编译。
