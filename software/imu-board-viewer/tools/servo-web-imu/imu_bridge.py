...
Validate behavior against the actual DLL/target logs. --resume explicitly opts
...
DEFAULT_FIRMWARE = PROJECT_ROOT / "firmware/imu_to_dxl/Build/imu_to_dxl.bin"
DEFAULT_MAP = DEFAULT_FIRMWARE.with_suffix(".map")
...
def validate_firmware_layout(binary: Path, map_file: Path) -> FirmwareLayout:
    ...
    sample = symbol("sample", SNAPSHOT_SIZE, 0x20000064, "imu.o")
    tick = symbol("tick_ms", 4, 0x20000BE0, "board.o")
...
class JLinkReader:
    ...
    def connect(self) -> None:
        ...
        if halted == 1:
            if not self.resume:
                raise BridgeError("目标 CPU 已暂停。固件已验证，可显式使用 --resume 恢复运行。")
...
class BridgeService:
    def __init__(self, *, demo: bool = False, resume: bool = False,
                 firmware: Path = DEFAULT_FIRMWARE, map_file: Path = DEFAULT_MAP,
    ...
    def disconnect(self) -> None:
        self.stop()
        self.publish(empty_frame("demo" if self.demo else "live", "disconnected", "连接已释放；可以返回 Keil 调试，或点击重新连接。"))
