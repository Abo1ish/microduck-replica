...
class ExplicitProbeConfigTests(unittest.TestCase):
    ...
    def test_direct_server_live_cli_rejects_missing_or_invalid_probe(self):
        from imu_server import parse_args
        for arguments in ([], ["--dll", str(self.dll_path)],
                          ["--probe-serial", "12345678"],
                          ["--probe-serial", "0", "--dll", str(self.dll_path)],
                          ["--probe-serial", "12345678", "--dll", str(self.dll_path.parent / "missing.dll")]):
            with self.subTest(arguments=arguments), redirect_stderr(io.StringIO()), self.assertRaises(SystemExit) as error:
                parse_args(arguments)
            self.assertEqual(error.exception.code, 2)

    def test_direct_server_demo_and_configured_live_cli(self):
        from imu_server import parse_args
        demo = parse_args(["--demo"])
        self.assertIsNone(demo.probe_serial)
        self.assertIsNone(demo.dll)
        live = parse_args(["--probe-serial", "12345678", "--dll", str(self.dll_path)])
        self.assertEqual(live.probe_serial, 12345678)
        self.assertEqual(live.dll, self.dll_path.resolve())


class ServerTests(unittest.TestCase):
    def test_demo_stream_and_local_control(self):
        from starlette.testclient import TestClient
        from imu_server import create_app
        service = bridge.BridgeService(demo=True)
        with TestClient(create_app(service), base_url="http://127.0.0.1:8081", client=("127.0.0.1", 12345)) as client:
            with client.websocket_connect("ws://127.0.0.1:8081/ws/imu", headers={"origin": "http://127.0.0.1:8081"}) as socket:
                frame = socket.receive_json()
                self.assertEqual(frame["mode"], "demo")
                self.assertTrue(frame["live"])
                self.assertFalse(frame["connected"])
            status = client.get("/api/status").json()
            self.assertEqual(status["service"], "microduck-imu-viewer")
            token_headers = {"x-imu-control-token": status["control_token"], "origin": "http://127.0.0.1:8081"}
            self.assertEqual(client.post("/api/disconnect").status_code, 403)
            self.assertEqual(client.post("/api/disconnect", headers=token_headers).status_code, 200)
            self.assertEqual(client.get("/api/status").json()["status"], "disconnected")
            self.assertEqual(client.post("/api/reconnect", headers=token_headers).status_code, 200)
            time.sleep(0.07)
            self.assertTrue(client.get("/api/status").json()["live"])
        self.assertFalse(service.thread.is_alive())

    def test_remote_origins_hosts_and_traversal_rejected(self):
        from starlette.testclient import TestClient
        from starlette.websockets import WebSocketDisconnect
        from imu_server import create_app, valid_origin
        self.assertFalse(valid_origin("https://evil.example", 8081))
        self.assertFalse(valid_origin("http://127.0.0.1:9999", 8081))
        service = bridge.BridgeService(demo=True)
        with TestClient(create_app(service), base_url="http://127.0.0.1:8081", client=("127.0.0.1", 12345)) as client:
            self.assertEqual(client.get("/api/status", headers={"origin": "https://evil.example"}).status_code, 403)
            self.assertEqual(client.get("/api/status", headers={"host": "evil.example"}).status_code, 400)
            with self.assertRaises(WebSocketDisconnect):
                with client.websocket_connect("ws://127.0.0.1:8081/ws/imu", headers={"origin": "https://evil.example"}):
                    self.fail("Remote-origin websocket accepted")
            self.assertEqual(client.get("/model/%2E%2E/imu_bridge.py").status_code, 404)
